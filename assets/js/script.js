...
var displayForecast = () => {
    ...
    var currentIcon = "http://openweathermap.org/img/w/" + forecastDataArray.list[0].weather[0].icon + ".png"
        ...
        currentWeatherTitleEl.classList.add("text-white", "bg-blue-800", "p-1", "mb-1", "w-full", "text-2xl")
        currentWeatherTitleEl.textContent = ` Weather now`        
        ...
        currentWeatherContainerEl.appendChild(currentCardEl);                                                                                                 
            ...
            currentCityEl.classList.add("forecast-top-rounded", "bg-blue-300", "font-bold", "text-3xl", "p-1", "text-center", "items-center", "justify-center", "w-4/5")                                                                       
            ...
            currentDateTimeEl.classList.add("bg-blue-300", "font-bold", "text-xl", "p-1", "text-center", "items-center", "justify-center", "w-4/5")                                                                       
            ...
            currentIconEl.classList.add("bg-blue-300", "flex", "justify-center", "text-center", "items-center", "w-4/5")
            currentCardEl.appendChild(currentIconEl);                                                                                          

                var currentIconLink = document.createElement('img')
                currentIconLink.classList.add("bg-blue-400", "text-center", "object-fill", "w-28", "items-center", "rounded-xl")                                                                       
                currentIconLink.src = currentIcon;                
                currentIconEl.appendChild(currentIconLink);  
               

            var currentTempEl = document.createElement('div')
            currentTempEl.classList.add("bg-blue-300", "w-4/5", "text-center", "text-xl", "p-1")
            ...
            currentMinMaxTempEl.classList.add("bg-blue-300", "w-4/5", "text-center", "text-xl")                                                                       
            ...
            currentWindEl.classList.add("bg-blue-300", "w-4/5", "text-center", "text-xl")                                                                       
    ...
    // Create heading for forcast//
    var forecastEl = document.createElement('h2');
    forecastEl.classList.add("text-white", "bg-blue-800", "p-1", "w-full", "text-2xl")
    forecastEl.textContent = "Forecast Weather:"        
    forecastContainerEl.appendChild(forecastEl);

    //- Render remaining forecast cards -//
    if (!arrayForRendering.index[5].length) { // assess how many cards need to be rendered (Number of days forecast varies depending when the API fetch is made)
        var forLoop = 5
    } else {
        var forLoop = 6
    }

    for (let i = 0; i < forLoop; i = i+1) {    
         
        var forecastCardEl = document.createElement('div');                                                                                             
        forecastCardEl.classList.add ("forecast-card", "rounded-xl", "flex", "flex-wrap", "bg-blue-600", "w-1/4", "justify-center", "flex-in-css-stylesheet", "p-1", "m-1"); 
        forecastContainerEl.appendChild(forecastCardEl);                                                                                                 
        
            var forecastDateEl = document.createElement('div')
            forecastDateEl.classList.add("forecast-top-rounded", "bg-blue-600", "w-full", "font-bold", "text-center", "text-blue-100", "pt-1", "weight-800", "text-xl")                                                                       
            forecastDateEl.textContent = arrayForRendering.index[i].date           
            forecastCardEl.appendChild(forecastDateEl);                                                                                         

            var forecastIconEl = document.createElement('div')
            forecastIconEl.classList.add("bg-blue-600", "w-full", "flex", "justify-center", "text-center", "text-blue-100", "p-1")
            forecastCardEl.appendChild(forecastIconEl);                                                                                          

                var forecastIconLink = document.createElement('img')
                forecastIconLink.classList.add("bg-blue-300", "object-fill", "rounded-xl", "w-24")                                                                       
                forecastIconLink.src = arrayForRendering.index[i].iconURL;                
                forecastIconEl.appendChild(forecastIconLink);     

            var forecastMaxTempEl = document.createElement('div')
            forecastMaxTempEl.classList.add("bg-blue-600", "w-full", "text-center", "text-blue-100")
            forecastMaxTempEl.textContent = arrayForRendering.index[i].maxTemp            
            forecastCardEl.appendChild(forecastMaxTempEl);    
           
            var forecastMinTempEl = document.createElement('div')
            forecastMinTempEl.classList.add("bg-blue-600", "w-full", "text-center", "text-blue-100")                                                                       
            forecastMinTempEl.textContent = arrayForRendering.index[i].minTemp
            forecastCardEl.appendChild(forecastMinTempEl);    

            var forecastMaxWindEl = document.createElement('div')
            forecastMaxWindEl.classList.add("bg-blue-600", "w-full", "text-center", "text-blue-100")                                                                       
            forecastMaxWindEl.textContent = arrayForRendering.index[i].maxWind;
            forecastCardEl.appendChild(forecastMaxWindEl);    

            var forecastMaxHumidityEl = document.createElement('div')
            forecastMaxHumidityEl.classList.add("forecast-bottom-rounded", "bg-blue-600", "w-full", "text-center", "text-blue-100", "pb-1");                                                                       
            forecastMaxHumidityEl.textContent = arrayForRendering.index[i].maxHumidity;
            forecastCardEl.appendChild(forecastMaxHumidityEl);   
    };
    renderHistoryButtons();            
    return;
};
...
var renderHistoryButtons = () => {
    ...
    if (previousSearchArray.length > 0) {        
        ...
        previousSearchContainerEl.hidden = false;    // Show the "history" container
    } else {        
        console.log("  Hiding Clear History Button");
        clearHistoryBtn.hidden = true;               // Hide the "clear history" button
        previousSearchContainerEl.hidden = true;     // Hide the "history" container
    }
    return;
};
